...
if __name__ == "__main__":
    ...
    for img_path in list_images_paths:
        ...
        if args.full_pipeline:
            ...
            logger.info("first pixel color check bf det grapes crop: {}".format(img_grapes_crop[0][0]))
            logger.info("first pixel color check bf det img_ori: {}".format(img_ori_np[0][0]))
            img_out_bbox, img_out_mask, mask, img_health, health_flag, cont_det, pred = predict_img(img_grapes_crop, args, model_predictor, save=True, save_path=output_folder, img_o=img_ori_np, fruit_zone=fruit_bbox, health_model=yolo_clss_health, cont_det=cont_det)
